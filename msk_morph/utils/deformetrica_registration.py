...
class DeformetricaRegistration:
    """
    A Python pipeline to run Deformetrica registration using the Python API directly.
    """
    ...
    def downsample_meshes(self,
        ...
        Args:
            ...
            reduction_factor: Target reduction factor (0.5 = 50% reduction)
            output_suffix: Suffix added to downsampled filenames
        ...
        for target_file in target_files:
            ...
            try:
                ...
                self.logger.info(
                    f"Simplified target - Points: {simplified_polydata.GetNumberOfPoints()}, "
                    f"Cells: {simplified_cells}, Reduction: {actual_reduction:.1f}%"
                )

                target_path = Path(target_file)
                downsampled_target = os.path.abspath(
                    os.path.join(output_dir, f"{target_path.stem}{output_suffix}{target_path.suffix}")
                )

                writer = vtk.vtkXMLPolyDataWriter() if target_file.endswith('.vtp') else vtk.vtkPolyDataWriter()
                writer.SetFileName(downsampled_target)
                writer.SetInputData(simplified_polydata)
                writer.Write()

                if not os.path.exists(downsampled_target):
        ...
        self.logger.info(f"Target mesh downsampling completed. Template (unchanged): {template_file}")
    ...
    def _run_registration_step(self,
        ...
        """
        Execute a single Deformetrica registration step.
        ...
        dataset_specifications = {
            "dataset_filenames": [[{"bone_trial": target_file}]],
            "subject_ids": ["subject_0"],
        }
        ...
        self.logger.info("Starting Deformetrica registration...")
        self.logger.info(f"  Template: {template_file}")
        self.logger.info(f"  Target:   {target_file}")
        self.logger.info(f"  Output:   {output_dir}")

        try:
            deformetrica = Deformetrica(output_dir=output_dir, verbosity=verbosity)
            ...
            self.logger.error(f"Deformetrica registration step failed: {e}")
            import traceback
    ...
    def run_single_mesh_registration(self,
        ...
        Supports two modes:
          - Single-step (iterative_reg=False): kernel widths are scalars.
          - Iterative / coarse-to-fine (iterative_reg=True): both kernel width
            arguments must be equal-length lists. The warped template output of
            stage N is used as the template for stage N+1.
        ...
        if iterative_reg:
            ...
            def_kw_list = [deformation_kernel_width[0] if isinstance(deformation_kernel_width, list)
                           else deformation_kernel_width]
            tmpl_kw_list = [template_kernel_width[0] if isinstance(template_kernel_width, list)
        ...
        if downsample_meshes:
            ...
            try:
                ...
                self.logger.error(f"Mesh downsampling failed: {e}. Proceeding with original meshes.")
        else:
        ...
        for step_idx in range(n_steps):
            ...
            if not success:
                self.logger.error(
                    f"Registration failed at step {step_idx + 1}/{n_steps}. Stopping."
                )
                return False
            ...
            if iterative_reg and step_idx < n_steps - 1:
                ...
                if warped_template is None:
                    ...
                    return False
                self.logger.info(f"Chaining warped template for next step: {warped_template}")
...
def find_mesh_pairs(aligned_meshes_dir: str) -> List[Tuple[str, str, str]]:
    ...
    template_files = glob.glob(os.path.join(aligned_meshes_dir, "*_template_aligned.vtk"))

    for template_file in template_files:
        mesh_name = os.path.basename(template_file).replace("_template_aligned.vtk", "")
        ...
        if os.path.exists(target_file):
            ...
            print(f"No target file found for {mesh_name}")
...
def run_batch_registration(aligned_meshes_dir: str,
    ...
    Args:
        ...
        deformation_kernel_width: Scalar (single-step) or list (iterative)
        template_kernel_width: Scalar (single-step) or list (iterative)
    ...
    pipeline = DeformetricaRegistration()
    ...
    for mesh_name, template_file, target_file in mesh_pairs:
        ...
        try:
            success = pipeline.run_single_mesh_registration(
                template_file=template_file,
                target_file=target_file,
            ...
            if success:
                logger.info(f"{mesh_name} registration completed in {processing_time:.2f}s")
